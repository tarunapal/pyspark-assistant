...
class MemoryManager {
    constructor() {
        this.executorMemoryWarningThreshold = 0.8; // 80% memory usage triggers warning
        this.executorMemoryUsage = new Map(); // executor-id -> memory usage (0-1)
    }
    setPySparkAnalyzer(analyzer) {
        this.pysparkAnalyzer = analyzer;
    }
    trackCellExecution(event) {
        ...
        for (const change of event.contentChanges) {
            ...
            const executedCells = cells.filter(cell => { var _a; return ((_a = cell.metadata) === null || _a === void 0 ? void 0 : _a.executionOrder) !== undefined; });
            for (const cell of executedCells) {
                this.analyzeCellForMemoryIssues(cell);
            }
        }
    }
    analyzeCellForMemoryIssues(cell) {
        const cellText = cell.document.getText();
        // Look for potential memory-intensive operations
        if (/\.collect\(\)|\.toPandas\(\)/.test(cellText) &&
            !/\.limit\(|\.sample\(/.test(cellText)) {
            vscode.window.showWarningMessage("Memory-intensive operation detected. Consider using .limit() or .sample() before collecting data to driver.", "Learn More").then(selection => {
                if (selection === "Learn More") {
                    vscode.env.openExternal(vscode.Uri.parse("https://spark.apache.org/docs/latest/tuning.html#memory-management-overview"));
                }
            });
        }
        // Look for join operations without broadcast hints on smaller DataFrames
        if (/\.join\(/.test(cellText) && !/broadcast\(/.test(cellText)) {
            vscode.window.showInformationMessage("Join operation detected. Consider using broadcast hint for smaller DataFrame to optimize memory usage.", "Learn More").then(selection => {
                if (selection === "Learn More") {
                    vscode.env.openExternal(vscode.Uri.parse("https://spark.apache.org/docs/latest/sql-performance-tuning.html#broadcast-hint-for-sql-queries"));
                }
            });
        }
    }
    updateExecutorMemoryUsage(executorId, memoryUsage) {
        ...
        if (memoryUsage > this.executorMemoryWarningThreshold) {
            vscode.window.showWarningMessage(`Executor ${executorId} is using ${Math.round(memoryUsage * 100)}% of its allocated memory, which may lead to OutOfMemoryError.`, "Show Recommendations").then(selection => {
    ...
    showMemoryRecommendations() {
        const panel = vscode.window.createWebviewPanel('memoryRecommendations', 'Memory Optimization Recommendations', vscode.ViewColumn.Beside, {});
        panel.webview.html = `
            <!DOCTYPE html>
            <html>
            <head>
