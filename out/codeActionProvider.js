...
exports.PySparkCodeActionProvider = void 0;
const vscode = require("vscode");
class PySparkCodeActionProvider {
    provideCodeActions(document, range, context, token) {
        const actions = [];
        for (const diagnostic of context.diagnostics) {
            if (diagnostic.code === 'collect-large-df') {
                const fix = new vscode.CodeAction('Replace with limit(1000).collect()', vscode.CodeActionKind.QuickFix);
            ...
            if (diagnostic.code === 'to-pandas-large-df') {
                const fix = new vscode.CodeAction('Replace with limit(1000).toPandas()', vscode.CodeActionKind.QuickFix);
                const text = document.getText(diagnostic.range);
            ...
            if (diagnostic.code === 'missing-cache') {
                ...
                if (!/\.cache\(\)/.test(line)) {
                    ...
                    if (match) {
                        ...
                        const fix = new vscode.CodeAction('Add .cache() to DataFrame assignment', vscode.CodeActionKind.QuickFix);
                        fix.edit = new vscode.WorkspaceEdit();
            ...
            if (diagnostic.code === 'cartesian-join') {
                ...
                const fix = new vscode.CodeAction('Replace with join and add join condition template', vscode.CodeActionKind.QuickFix);
                fix.edit = new vscode.WorkspaceEdit();
                fix.edit.replace(document.uri, diagnostic.range, fixedText);
                fix.diagnostics = [diagnostic];
                fix.isPreferred = true;
            ...
            if (diagnostic.code === 'broadcast-hint') {
                ...
                const fix = new vscode.CodeAction('Use F.broadcast() for small DataFrame in join', vscode.CodeActionKind.QuickFix);
                fix.edit = new vscode.WorkspaceEdit();
                fix.edit.replace(document.uri, diagnostic.range, fixedText);
                fix.diagnostics = [diagnostic];
                fix.isPreferred = true;
...
exports.PySparkCodeActionProvider = PySparkCodeActionProvider;
PySparkCodeActionProvider.providedCodeActionKinds = [
